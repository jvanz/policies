...
@test "reject because a required label does not exist with ingress without labels" {
	run kwctl run policy.wasm \
		-r test_data/ingress-no-labels.json \
		--settings-json '{"criteria": "doesNotContainOtherThan" ,"values": ["foo", "bar"]}'
...
@test "reject because empty label list on settings" {
	run kwctl run policy.wasm \
		-r test_data/ingress-no-labels.json \
	...
	# incorrect settings
	[ "$status" -eq 1 ]
	[ $(expr "$output" : '.*Provided settings are not valid: Empty label list is not allowed.*') -ne 0 ]
}
